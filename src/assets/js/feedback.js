	...
	$.feedback = function(options) {
			...
			var settings = $.extend({
							...
							tpl: {
									...
									highlighter:    '<div id="feedback-highlighter"><div class="feedback-logo">問題回報</div><p>幫助我們了解您的問題。(此視窗可拖曳)</p><button class="feedback-setblackout"><div class="ico"></div><span>遮蔽有關個資內容</span></button><button class="feedback-sethighlight feedback-active"><div class="ico"></div><span>選取有關問題內容</span></button><div class="feedback-buttons"><button id="feedback-highlighter-next" class="feedback-next-btn feedback-btn-gray">下一步</button><button id="feedback-highlighter-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div class="feedback-wizard-close"></div></div>',
									overview:       '<div id="feedback-overview"><div class="feedback-logo">問題回報</div><div id="feedback-overview-description"><div id="feedback-overview-description-text"><h3>問題描述</h3><!--<h3 class="feedback-additional">Additional info</h3><div id="feedback-additional-none"><span>None</span></div><div id="feedback-browser-info"><span>Browser Info</span></div><div id="feedback-page-info"><span>Page Info</span></div><div id="feedback-timestamp"><span>Time Stamp</span></div><div id="feedback-page-structure"><span>Page Structure</span></div>--></div></div><div id="feedback-overview-screenshot"><h3>畫面截圖</h3></div><div class="feedback-buttons"><button id="feedback-submit" class="feedback-submit-btn feedback-btn-blue">送出</button><button id="feedback-overview-back" class="feedback-back-btn feedback-btn-gray">上一步</button></div><div id="feedback-overview-error">請填寫問題描述</div><div class="feedback-wizard-close"></div></div>',
									submitSuccess:  '<div id="feedback-submit-success"><div class="feedback-logo">問題回報</div><p>感謝您的回報，我們將會盡快回覆您，謝謝。</p><button class="feedback-close-btn feedback-btn-blue">確認</button><div class="feedback-wizard-close"></div></div>',
									submitError:    '<div id="feedback-submit-error"><div class="feedback-logo">問題回報</div><p>送出時發生錯誤，請稍候再試。</p><button class="feedback-close-btn feedback-btn-blue">確認</button><div class="feedback-wizard-close"></div></div>'
							},
							onSubmit:               function() {},
					...
					if (supportedBrowser) {
							if(isFeedbackButtonNative) {
									$('body').append('<div id="feedback-button"><button class="feedback-btn feedback-btn-gray">' + settings.initButtonText + '</button></div>');
							}
							$(document).on('click', settings.feedbackButton, function(){
									...
									$(document).on('mouseup', '#feedback-canvas', function(){
											if (canDraw) {
													drag = false;
									...
									$(document).on('selectstart dragstart', '#feedback-canvas', function(e) {
											e.preventDefault();
									});
											...
											if ($('#feedback-note').val().length > 0) {
													...
													$.ajax({
															...
															success: function() {
																	...
																	settings.afterSubmit.call(this);
															},
															error: function(e){
																	console.log(e);
